...
DRUGNAMES_DIR = str(BASE_DIR / "db" / "chroma_drugnames")
...
async def ocr_image(file: UploadFile = File(...)):
    ...
    try:
        ...
        raw_clean = re.sub(r"[^a-zA-Z0-9\s\+\-]", " ", raw).lower()
        raw_clean = re.sub(r"\s+", " ", raw_clean).strip()

        # tokens (ignore tiny words)
        tokens = [t for t in raw_clean.replace("+", " ").split() if len(t) > 2]

        # --- Chroma drug name correction ---
        corrected = []
        debug = []

        for t in tokens:
            best, dist = correct_token_with_chroma(t)

            # distance threshold: tune this if needed
            # small distance is good.
            if best and dist < OCR_DISTANCE_THRESHOLD:
                corrected.append(best)
                debug.append({"token": t, "best": best, "dist": dist})
            else:
                debug.append({"token": t, "best": best, "dist": dist})
        ...
        # Fallback: if strict correction found nothing, return cleaned tokens.
    ...
    try:
        try:
            ...
            if not hasattr(stt_audio, "_model"):
                stt_audio._model = WhisperModel("base", device="cpu", compute_type="int8")

            segments, info = stt_audio._model.transcribe(tmp_path)
            text = " ".join(seg.text.strip() for seg in segments).strip()
            return {"text": text, "language": getattr(info, "language", None), "mode": "faster-whisper"}
        except Exception as e:
